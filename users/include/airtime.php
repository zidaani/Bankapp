...
$pin = mysqli_real_escape_string($conn,$_POST['pin']);
      ...
      if ($pin != $pincheck) {
 header('Location: ../buy_airtime.php?msgr=wrong pin code');
 exit();
}

if (empty($pin)) {
 header('Location: ../buy_airtime.php?msgr=pin code empty');
 exit();
}
...
if($amount>$balancecheck){
   header('Location: ../buy_airtime.php?msgr=insufficient balance! Please top up');
 exit();    
}
...
header('Location: ../buy_airtime.php?msgr=unsuccessful transaction');
