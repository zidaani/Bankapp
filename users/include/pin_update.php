...
if (isset($_POST['pinchange'])) {
  ...
  if ($pin != $pin1) {
     ...
     header('Location: ../edit_profile.php?msgr=pin unmatch');
   
    exit();
  }

 if (empty($oldpin)) {
  header('Location: ../edit_profile.php?msgr=empty old pin');
    
    exit();
   }

   if (empty($pin)) {
  header('Location: ../edit_profile.php?msgr=empty first pin');
    
    exit();
   }

   if (empty($pin1)) {
  header('Location: ../edit_profile.php?msgr=confirm pin');
  ...
  if ($pincheck != $oldpin) {
   header('Location: ../edit_profile.php?msgr=wrong old pin');
 ...
 if ($sqllrow==1) {
  ...
  $sqll3 = "UPDATE users SET pin = '$pin'  WHERE u_id = '$u_id' ";
   ...
   header('Location: ../edit_profile.php?msgr=unsuccessful profile update');
