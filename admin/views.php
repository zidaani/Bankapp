...
mysqli_query($conn, "UPDATE support SET admin_status='read', reply_s_id='$sid' WHERE s_id='$sid' AND sender_id='$sender_id' ");
...
$si_u_name=$_SESSION['s_u_name'];
    ...
    $jey="INSERT INTO support (message,u_id,receiver_id,sender_id,sender,reply_s_id,admin_status,status,message_type,s_u_name) VALUES('$reply_message','$u_id','$sender_iid','$u_id','$si_u_name','$sid','read','unread','replied','$username')";
	$admin_reply =mysqli_query($conn, $jey);

     $f= mysqli_query($conn,"SELECT * FROM support");

     $update_replies="UPDATE support SET admin_status='read' WHERE message_type='replied' AND reply_s_id='$sid' ";
