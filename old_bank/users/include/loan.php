...
if (empty($pin)) {
 header('Location: ../get_loan.php?msg=pin code empty');
 exit();
}

      if ($pin != $pincheck) {
 header('Location: ../get_loan.php?msg=wrong pin code');
 exit();
}


if (empty($loan_terms)) {
	header('Location: ../get_loan.php?msg=accept terms');
	exit();
}
...
header('Location: ../get_loan.php?msg=unsuccessful transaction');
