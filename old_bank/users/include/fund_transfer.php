...
if (isset($_POST['transfer'])) {
   ...
   $pin = mysqli_real_escape_string($conn,$_POST['pin']);
      ...
      if ($pin != $pincheck) {
 header('Location: ../fund_transfer.php?msg=wrong pin code');
 exit();
}

     if ($a_number == $receiver) {
 header('Location: ../fund_transfer.php?msg=can not send to yourself');
 exit();
}
...
if ($tran3 != 1) {
   header('Location: ../fund_transfer.php?msg=account number not found');
 exit();
  }


if (empty($pin)) {
 header('Location: ../fund_transfer.php?msg=pin code empty');
 exit();
}
